# Instructions: XLight Child Paper Release & Tagging Workflow

## Context
This workflow governs versioning, milestone validation, release stamping, and git tagging for the dedicated XLight child paper [`child_papers/xlight.md`](../xlight.md) **strictly within the `child_papers/` directory**.
...
- Inspect [`child_papers/xlight.md`](../xlight.md) to ensure all core technical sections are present, deeply analyzed, and fully populated:
...
### 2. Release Stamping
- Stamp the version header in [`child_papers/xlight.md`](../xlight.md) under the document metadata block:
  ```markdown
...
- Commit the release with Conventional Commits syntax:
  ```bash
  git commit -m "release(xlight): release version <VERSION> of xlight.md child paper"
  ```
- Create an annotated Git tag for the child paper release:
  ```bash
  git tag -a xlight-v<VERSION> -m "xlight.md child paper release v<VERSION>"
  ```
...
- Release Version (e.g. `xlight-v1.0.0`)
- Tag created in Git
