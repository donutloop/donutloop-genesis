...
* **Detailed Profile (§3.1):** Add a dedicated entry for the entity under `### 3.1 Industry, Hyperscale & Hardware Commitments`. Each entry must open with a lead-in line giving the entity's full legal/brand name, a one-line identifying description (location, sector, product lines), its specific role on the relevant Genesis Mission project, and an inline citation with source name and link to the primary announcement or program page. The entry must then continue with the following bold sub-bullets, in this order:
