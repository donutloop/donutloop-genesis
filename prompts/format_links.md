...
- **Coverage Reference Check (MANDATORY):** After integrating WIP entries into `references.md`, extend `reference_coverage.md` to include every newly added link:
  1. Add new index rows to the master reference link table in the correct section, with sequential numbering and `Unprocessed` status.
  2. Renumber all subsequent rows across all sections to maintain a continuous sequence.
  3. Update the executive summary metrics: total reference links, unique target domains, web portals & press releases count, processing status ratio, and section distribution table (link counts, share percentages).
